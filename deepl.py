...
AUTH_KEY = "a29fd6b6-977e-b585-3148-25a7dfa9ec91:fx"
...
def docx_zh2en(file_path):
    ...
    zh_docx_translator(file_path, output_path, "EN", AUTH_KEY)

def en_docx_translator(input_path, output_path, target_language, auth_key=AUTH_KEY):
...
def zh_docx_translator(input_path, output_path, target_language, auth_key=AUTH_KEY):
...
def array_translator(sentences, target_language, auth_key=AUTH_KEY):
    translated_array = []
...
#测试API
def translate_with_deepl(text, target_language, auth_key=AUTH_KEY):
    # 定义一个静态变量作为计数器
    if not hasattr(translate_with_deepl, "counter"):
        translate_with_deepl.counter = 0
    ...
    # 增加计数器的值并打印
    translate_with_deepl.counter += 1
    print(f"{translate_with_deepl.counter}.{response.text}")
